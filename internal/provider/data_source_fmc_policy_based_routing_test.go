...
func TestAccDataSourceFmcPolicyBasedRouting(t *testing.T) {
	...
	checks = append(checks, resource.TestCheckResourceAttr("data.fmc_policy_based_routing.test", "description", "By provider"))
	checks = append(checks, resource.TestCheckResourceAttr("data.fmc_policy_based_routing.test", "forwarding_actions.0.forwarding_action_type", "SET_EGRESS_INTF_BY_PRIORITY"))
	checks = append(checks, resource.TestCheckResourceAttr("data.fmc_policy_based_routing.test", "forwarding_actions.0.match_criteria_name", "box-acl"))
	checks = append(checks, resource.TestCheckResourceAttr("data.fmc_policy_based_routing.test", "forwarding_actions.0.match_criteria_id", "000C29A7-0BEE-0ed3-0000-008589983100"))
	checks = append(checks, resource.TestCheckResourceAttr("data.fmc_policy_based_routing.test", "forwarding_actions.0.default_interface", "true"))
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		Steps: []resource.TestStep{
			{
				Config: testAccDataSourceFmcPolicyBasedRoutingPrerequisitesConfig + testAccDataSourceFmcPolicyBasedRoutingConfig(),
				Check:  resource.ComposeTestCheckFunc(checks...),
			},
			{
				Config: testAccDataSourceFmcPolicyBasedRoutingPrerequisitesConfig + testAccNamedDataSourceFmcPolicyBasedRoutingConfig(),
				Check:  resource.ComposeTestCheckFunc(checks...),
			},
		},
	})
...
const testAccDataSourceFmcPolicyBasedRoutingPrerequisitesConfig = `
...
func testAccDataSourceFmcPolicyBasedRoutingConfig() string {
	...
	config += `
		data "fmc_policy_based_routing" "test" {
			id = fmc_policy_based_routing.test.id
			device_id = var.device_id
		}
	`
	return config
...
func testAccNamedDataSourceFmcPolicyBasedRoutingConfig() string {
	config := `resource "fmc_policy_based_routing" "test" {` + "\n"
	config += `	device_id = var.device_id` + "\n"
	config += `	description = "By provider"` + "\n"
	config += `	ingress_interfaces = [{` + "\n"
	...
	config += `
		data "fmc_policy_based_routing" "test" {
			name = fmc_policy_based_routing.test.name
			device_id = var.device_id
		}
	`
	return config
